...
export class LocalStorageStrategy extends PersistenceStrategy {
  ...
  // Check if any local data exists without loading it (optimized for migration check)
  async hasAnyData() {
    try {
      ...
      for (const key of keys) {
        ...
        if (item && item !== 'null' && item !== '[]' && item !== '{}') {
          try {
            ...
            // If parsing fails but item exists, consider it as data
            return true;
          }
        }
      }
      ...
      console.error('Error checking for local data existence:', error);
      return false;
    }
  }
