...
const ProgressTracker = ({ calculationId }) => {
  ...
  useEffect(() => {
    const calculation = savedResults.find(calc => calc.id === calculationId);
    ...
    const generateProgressData = () => {
      ...
      for (let day = 0; day <= daysSinceStart; day++) {
        ...
        // Calculate quantity considering mortality
        let currentQuantity = calculation.quantity;
        if (calculation.mortalityLog) {
          calculation.mortalityLog.forEach(log => {
            const logDate = new Date(log.date);
            if (logDate <= currentDate) {
              currentQuantity -= log.deaths;
            }
          });
        }
        
        const dayCalc = calculateNextDay(calculation.id);
        if (dayCalc) {
          progressPoints.push({
            day,
            date: currentDate.toISOString().split('T')[0],
            age: currentAge,
            quantity: Math.max(0, currentQuantity),
            feedPerBird: dayCalc.feedPerBirdGrams,
            totalFeed: dayCalc.totalFeedKg,
            feedType: dayCalc.feedType,
            protein: dayCalc.protein,
            feedEfficiency: currentQuantity > 0 ? (dayCalc.totalFeedKg * 1000) / currentQuantity : 0,
            survivalRate: (currentQuantity / calculation.quantity) * 100
          });
        }
      }
      ...
      setProgressData({
        ...
        summary: {
          ...
          totalFeedConsumed: progressPoints.reduce((sum, point) => sum + point.totalFeed, 0),
          avgFeedEfficiency: progressPoints.length > 0 ? 
            progressPoints.reduce((sum, point) => sum + point.feedEfficiency, 0) / progressPoints.length : 0
        }
      });
    };
